...
#include "BrushBuilderToolController3D.h"
...
BrushBuilderToolController3D::BrushBuilderToolController3D(BrushBuilderTool& tool)
  : m_tool{tool}
{
}

Tool& BrushBuilderToolController3D::tool()
{
  return m_tool;
}

const Tool& BrushBuilderToolController3D::tool() const
{
  return m_tool;
}

void BrushBuilderToolController3D::pick(
  const InputState& inputState, mdl::PickResult& pickResult)
{
  m_tool.pick(inputState.pickRay(), inputState.camera(), pickResult);
}

bool BrushBuilderToolController3D::mouseClick(const InputState& inputState)
{
  ...
  const auto plane = m_tool.shapePlane().value_or(
    vm::plane3d{inputState.defaultPointUnderMouse(), vm::vec3d{camera.direction()}});
...
bool BrushBuilderToolController3D::mouseDoubleClick(const InputState& inputState)
{
  if (
    !inputState.mouseButtonsPressed(MouseButtons::Left)
    || !inputState.modifierKeysPressed(ModifierKeys::None))
...
std::unique_ptr<GestureTracker> BrushBuilderToolController3D::acceptMouseDrag(
  const InputState& inputState)
{
  using namespace mdl::HitFilters;
...
void BrushBuilderToolController3D::render(
  const InputState& inputState,
  render::RenderContext& renderContext,
...
bool BrushBuilderToolController3D::cancel()
{
